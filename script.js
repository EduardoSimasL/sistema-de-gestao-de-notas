    ...
    if (tbody) {
      ...
      if (alunosArmazenados) {
        ...
        alunos.forEach(aluno => {
          ...
          btnEditar2.textContent = 'Editar 2� Bimestre';
          btnEditar2.onclick = function() {
            if (verificarNotasBimestre1Adicionadas(aluno)) {
              editarNotasBimestre2(aluno);
            } else {
              alert("Por favor, preencha as notas do 1� Bimestre primeiro.");
            }
          };
      ...
      console.log("A tabela n�o possui um corpo (tbody).");
    }
  } else {
    console.log("A tabela n�o foi encontrada.");
  }
}
...
function excluirAluno(aluno) {
  ...
  if (alunosArmazenados) {
    ...
    if (index !== -1) {
      ...
      localStorage.setItem('alunos', JSON.stringify(alunosArmazenados));
      carregarDadosDaTabela(); // Recarrega os dados da tabela ap�s a exclus�o
    }
  }
}
